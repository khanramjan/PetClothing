...
try
{
    ...
    using var categoryCmd = new NpgsqlCommand(categorySql, connection);
    var categoriesAffected = await categoryCmd.ExecuteNonQueryAsync();
    Console.WriteLine($"✓ Inserted {categoriesAffected} categories");
    ...
    using var productCmd = new NpgsqlCommand(productSql, connection);
    var productsAffected = await productCmd.ExecuteNonQueryAsync();
    Console.WriteLine($"✓ Inserted {productsAffected} sample products");
    ...
    using var imageCmd = new NpgsqlCommand(imageSql, connection);
    var imagesAffected = await imageCmd.ExecuteNonQueryAsync();
    Console.WriteLine($"✓ Inserted {imagesAffected} product images");

    Console.WriteLine("\n=== Seeding Complete! ===");
    Console.WriteLine("Categories available:");
    Console.WriteLine("  - Dog Clothing");
    Console.WriteLine("  - Cat Clothing");
    Console.WriteLine("  - Bird Accessories");
    Console.WriteLine("  - Pet Costumes");
}
catch (Exception ex)
{
    Console.WriteLine($"\n✗ Error: {ex.Message}");
